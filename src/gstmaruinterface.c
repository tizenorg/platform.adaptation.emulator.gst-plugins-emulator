...
static int
init_codec (int fd, CodecContext *ctx)
{
  GST_DEBUG (" >> Enter");
//  CodecContext *cc = g_malloc(sizeof(CodecContext));

  ret = ioctl(fd, IOCTL_RW(IOCTL_CMD_INIT_CODEC), ctx);
  return ret;
}
static int
...
init (CodecContext *ctx, CodecElement *codec, CodecDevice *dev)
{
  ...
  //S_FMT
  //CodecContent->codectype, name
  //CodecContext
  //
  //& G_FMT
  //CodecContext->audio.sample_fmt,
  //                   .frame_size,
  //                   .bits_per_sample_fmt
  //            ->codecdata_size,
  //            ->codecdata

  ret = init_codec (dev->fd, ctx);

  if (ret < 0) {
    GST_ERROR ("failed to init codec");
    return -1;
  }
